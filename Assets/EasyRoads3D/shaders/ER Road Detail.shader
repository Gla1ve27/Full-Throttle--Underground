	...
	SubShader
	{
		...
		Pass
		{
						...
						)
			{
				...
				float3 unpack52 = UnpackNormalScale( tex2D( _BumpMap, texCoord47 ), _BumpScale );
				unpack52.z = lerp( 1, unpack52.z, saturate(_BumpScale) );
				float2 texCoord48 = packedInput.ase_texcoord5.zw * float2( 1,1 ) + float2( 0,0 );
				float3 unpack57 = UnpackNormalScale( tex2D( _DetailNormalMap, texCoord48 ), _DetailNormalMapScale );
				unpack57.z = lerp( 1, unpack57.z, saturate(_DetailNormalMapScale) );
				
				float2 uv_Metallic = packedInput.ase_texcoord5.xy * _Metallic_ST.xy + _Metallic_ST.zw;
				float4 tex2DNode55 = tex2D( _Metallic, uv_Metallic );
				
				surfaceDescription.BaseColor = ( ( ( tex2DNode43 * _Color ) * ( tex2D( _DetailAlbedoMap, texCoord41 ) * _Color1 ) ) * float4(2,2,2,2) ).rgb;
				surfaceDescription.Normal = BlendNormal( unpack52 , unpack57 );
				surfaceDescription.BentNormal = float3( 0, 0, 1 );
				surfaceDescription.CoatMask = 0;
				surfaceDescription.Metallic = ( tex2DNode55.r * _MainMetallicPower );
				...
				surfaceDescription.Smoothness = ( tex2DNode55.a * _MainSmoothnessPower );
				surfaceDescription.Occlusion = ( tex2DNode55.g * _OcclusionStrength );
				surfaceDescription.Alpha = tex2DNode43.a;
		...
		Pass
		{
						...
						)
			{
				...
				float3 unpack52 = UnpackNormalScale( tex2D( _BumpMap, texCoord47 ), _BumpScale );
				unpack52.z = lerp( 1, unpack52.z, saturate(_BumpScale) );
				float2 texCoord48 = packedInput.ase_texcoord7.zw * float2( 1,1 ) + float2( 0,0 );
				float3 unpack57 = UnpackNormalScale( tex2D( _DetailNormalMap, texCoord48 ), _DetailNormalMapScale );
				unpack57.z = lerp( 1, unpack57.z, saturate(_DetailNormalMapScale) );
